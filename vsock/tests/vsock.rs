...
#[tokio::test]
async fn async_echo() {
    let listener = VSockListener::bind(&SocketAddr::new(VMADDR_CID_ANY, VMADDR_PORT_ANY)).unwrap();
    let loc = listener.local_addr().unwrap();
    futures::join!(
        async {
...
async fn async_echo_poll() {
    ...
    futures::join!(
        ...
        async {
            let data: &[u8; 8] = b"datadata";
            let mut client = VSockStream::connect(&SocketAddr::new(VMADDR_CID_LOCAL, loc.port()))
                .await
                .unwrap();
